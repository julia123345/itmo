package app.commands;

import app.Client;
import app.UserRequest;
import app.commands.types.ClientCommand;
import app.model.Person;
import app.util.requests.PersonRequest;

import java.io.IOException;

/**
 * Клиентская команда remove_greater {element}:
 * формирует объект Person из ввода пользователя и отправляет его на сервер.
 */
public class RemoveGreaterCommand extends ClientCommand {

    public RemoveGreaterCommand(Client client) {
        super(client,
                "remove_greater",
                "Удаляет элементы коллекции, превышающие заданный",
                CommandType.REMOVE_GREATER);
    }

    @Override
    public boolean execute(String line, String[] arguments) {
        if (arguments.length != 0) {
            getConsole().println("Неверное использование команды.");
            return false;
        }
        Person person = (Person) new PersonRequest(getConsole()).create();
        if (person == null) return false;
        try {
            getClient().requestToServer(new UserRequest(getClient().getUser(), line, person));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        return true;
    }
}

